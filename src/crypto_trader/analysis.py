...
def detect_regime(df: pd.DataFrame) -> Regime:
    x=enrich(df); r=x.iloc[-1]; adx=float(r.adx14) if pd.notna(r.adx14) else 0.0
    ...
    momentum="bullish" if r.ema20>r.ema50 and r.macd_hist>0 else "bearish" if r.ema20<r.ema50 and r.macd_hist<0 else "mixed"
    return Regime(name,round(max(0.0,min(100.0,adx)),2),vol,momentum)

def fibonacci(df: pd.DataFrame, lookback: int=200)->dict:
    x=df.tail(lookback); hi,lo=float(x.high.max()),float(x.low.min())
    if hi<=lo:return {}
    span=hi-lo; return {str(k):hi-span*k for k in (0.236,0.382,0.5,0.618,0.786)}

def volume_profile(df: pd.DataFrame,bins: int=24,lookback: int=300)->dict:
    x=df.tail(lookback); lo,hi=float(x.low.min()),float(x.high.max())
    if x.empty or hi==lo:return {}
    step=(hi-lo)/bins; buckets=[0.0]*bins
    for row in x.itertuples(): buckets[min(bins-1,max(0,int((float(row.close)-lo)/step)))]+=float(row.volume)
    poc=max(range(bins),key=buckets)
    return {"low":lo,"high":hi,"bins":bins,"poc":lo+(poc+0.5)*step,"total_volume":sum(buckets)}

def multi_timeframe(data: dict[str,pd.DataFrame])->dict:
    out={}
    for tf,df in data.items():
        x=enrich(df); r=x.iloc[-1]
        out[tf]={"close":float(r.close),"ema20":float(r.ema20) if pd.notna(r.ema20) else None,"ema50":float(r.ema50) if pd.notna(r.ema50) else None,"ema200":float(r.ema200) if pd.notna(r.ema200) else None,"rsi14":float(r.rsi14) if pd.notna(r.rsi14) else None,"atr14":float(r.atr14) if pd.notna(r.atr14) else None,"adx14":float(r.adx14) if pd.notna(r.adx14) else None,"macd_hist":float(r.macd_hist) if pd.notna(r.macd_hist) else None,"rel_volume":float(r.rel_volume) if pd.notna(r.rel_volume) else None}
    return out

def orderbook_metrics(depth: dict,levels: int=20)->dict:
    bids=[(float(p),float(q)) for p,q in depth.get("bids",[])[:levels]]; asks=[(float(p),float(q)) for p,q in depth.get("asks",[])[:levels]]
