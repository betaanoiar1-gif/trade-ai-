...
def detect_regime(df: pd.DataFrame) -> Regime:
    ...
    if atr_pct >= 5: vol = "high"
    elif atr_pct <= 1: vol = "low"
    else: vol = "normal"
    if adx >= 25:
        name = "trend"
    elif vol == "high":
...
def volume_profile(df: pd.DataFrame, bins: int = 24, lookback: int = 300) -> dict:
    x = df.tail(lookback)
    if x.empty or x.high.max() == x.low.min(): return {}
    edges = pd.Series(pd.np.linspace(x.low.min(), x.high.max(), bins + 1)) if False else None
    lo, hi = float(x.low.min()), float(x.high.max())
    step = (hi - lo) / bins
    buckets = [0.0] * bins
    for row in x.itertuples():
        idx = min(bins - 1, max(0, int((float(row.close) - lo) / step)))
        buckets[idx] += float(row.volume)
...
def multi_timeframe(data: dict[str, pd.DataFrame]) -> dict:
    ...
    for tf, df in data.items():
        ...
        r = x.iloc[-1]
        out[tf] = {
            "close": float(r.close), "ema20": float(r.ema20) if pd.notna(r.ema20) else None,
...
def orderbook_metrics(depth: dict, levels: int = 20) -> dict:
    ...
    return {"available": True, "best_bid": bids[0][0], "best_ask": asks[0][0], "mid": mid,
...
def derivative_metrics(funding: list, oi: list) -> dict:
    latest_f = funding[0] if funding else {}
    latest_o = oi[0] if oi else {}
    try: fr = float(latest_f.get("fundingRate", 0.0))
    except (TypeError, ValueError): fr = 0.0
    try: oi_now = float(latest_o.get("openInterest", 0.0))
    except (TypeError, ValueError): oi_now = 0.0
    prev = None
    if len(oi) > 1:
        try: prev = float(oi[1].get("openInterest", 0.0))
        except (TypeError, ValueError): pass
    return {"funding_rate": fr, "open_interest": oi_now,
            "open_interest_change_pct": ((oi_now / prev) - 1) * 100 if prev else None,
            "available": bool(funding or oi)}
