...
def validate_plan(
    ...
    if plan.decision not in {"LONG", "SHORT"}:
        return RiskDecision(False, "no executable trade decision")
    if equity <= 0:
        return RiskDecision(False, "non-positive equity")
    if daily_pnl <= -equity * limits.max_daily_loss:
        return RiskDecision(False, "daily loss limit reached")
    if plan.stop is None or price is None:
        return RiskDecision(False, "entry price and stop are required")
    entry = price if plan.entry_low is None or plan.entry_high is None else (plan.entry_low + plan.entry_high) / 2
    if entry <= 0 or plan.stop <= 0:
        return RiskDecision(False, "invalid entry or stop")
    if plan.decision == "LONG" and plan.stop >= entry:
        return RiskDecision(False, "long stop must be below entry")
    if plan.decision == "SHORT" and plan.stop <= entry:
    ...
    if plan.take_profit_1 is not None:
        ...
        if reward <= 0 or reward / distance < 1.0:
            return RiskDecision(False, "take profit has inadequate reward/risk")
