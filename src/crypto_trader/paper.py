...
class PaperBroker:
    ...
    def open(self, symbol: str, side: str, qty: float, price: float, stop: float, tp1=None, tp2=None):
        if qty <= 0 or symbol in self.positions or side not in {"long", "short"}:
            return False
        fill = self._fill(price, "buy" if side == "long" else "sell")
        notional = fill * qty
        fee = notional * self.fee_rate
        if notional > self.cash * self.max_leverage or fee > self.cash:
            return False
        self.cash -= fee
    ...
    def equity(self, marks: dict[str, float]) -> float:
        return self.cash + sum(
            self.unrealized(s, marks.get(s, p.entry)) for s, p in self.positions.items()
        )
    ...
    def apply_funding(self, rates: dict[str, float]):
        ...
        for symbol, p in self.positions.items():
            rate = float(rates.get(symbol, 0.0))
            payment = p.entry * p.qty * rate
    ...
    def check_exits(self, marks: dict[str, float]) -> list[dict]:
        ...
        for symbol, p in list(self.positions.items()):
            ...
            hit_tp = p.take_profit_1 is not None and (
                (p.side == "long" and mark >= p.take_profit_1)
                or (p.side == "short" and mark <= p.take_profit_1)
            )
            if hit_liq:
                pnl = self.close(symbol, liq or mark)
                events.append({"symbol": symbol, "reason": "liquidation", "pnl": pnl})
            elif hit_stop:
                pnl = self.close(symbol, p.stop)
                events.append({"symbol": symbol, "reason": "stop", "pnl": pnl})
            elif hit_tp:
                pnl = self.close(symbol, p.take_profit_1)
                events.append({"symbol": symbol, "reason": "take_profit", "pnl": pnl})
        return events
