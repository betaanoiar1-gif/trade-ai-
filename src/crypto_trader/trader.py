...
class CryptoTrader:
    ...
    def snapshot(self, interval="1h", limit=500):
        ...
        funding = self.bybit.funding(self.symbol)
        oi = self.bybit.open_interest(self.symbol)
        tech = {
            k: (None if last[k] != last[k] else float(last[k]))
            for k in [
                "ema20", "ema50", "ema200", "rsi14", "atr14", "adx14", "vwap", "macd",
                "macd_signal", "macd_hist", "bb_high", "bb_low", "rel_volume", "volume_z",
            ]
        }
        return {
            "symbol": self.symbol,
            ...
            "data_age_seconds": data_age_seconds(last.close_time),
            "price": float(last.close),
            "technical": tech,
            "structure": market_structure(df).__dict__,
            "regime": detect_regime(base).__dict__,
            "multi_timeframe": multi_timeframe(frames),
            "orderbook": orderbook_metrics(depth),
            "derivatives": derivative_metrics(funding, oi),
            "fibonacci": fibonacci(base),
            "volume_profile": volume_profile(base),
        }

    def _account_equity(self) -> float:
        account = self.state.get("paper_account") or {}
        try:
            return max(0.0, float(account.get("equity", self.limits.starting_equity)))
        except (TypeError, ValueError):
            return self.limits.starting_equity

    def analyze(self):
        try:
            snap = self.snapshot()
        except MarketDataError as exc:
            return {"mode": "blocked", "decision": "NO_TRADE", "reason": str(exc)}
        if snap["data_age_seconds"] > 180:
            return {
                "mode": "blocked",
                "decision": "NO_TRADE",
                "snapshot": snap,
        ...
        if not self.ai.enabled():
            return {
                ...
                "snapshot": snap,
                "decision": "WAIT",
                "reason": "AI gateway not configured; no autonomous decision is fabricated.",
            }
        try:
            raw = self.ai.chat(TRADER_SYSTEM, decision_prompt(snap))
            plan = self.ai.extract_plan(raw)
        except AIError as exc:
            return {"mode": "blocked", "snapshot": snap, "decision": "NO_TRADE", "reason": str(exc)}
