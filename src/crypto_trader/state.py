...
class StateStore:
    ...
    def log_decision(self, ts: str, symbol: str, payload: dict):
        self.db.execute(
            "INSERT INTO decisions(ts,symbol,payload) VALUES(?,?,?)",
            (ts, symbol, json.dumps(payload, default=str)),
        )
        self.db.commit()

    def log_trade(self, ts: str, symbol: str, action: str, payload: dict):
        self.db.execute(
            "INSERT INTO trades(ts,symbol,action,payload) VALUES(?,?,?,?)",
            (ts, symbol, action, json.dumps(payload, default=str)),
        )
        self.db.commit()

    def recent_decisions(self, limit: int = 20):
        rows = self.db.execute(
            "SELECT ts,symbol,payload FROM decisions ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
        return [{"ts": r[0], "symbol": r[1], "payload": json.loads(r[2])} for r in rows]

    def recent_trades(self, limit: int = 20):
        rows = self.db.execute(
            "SELECT ts,symbol,action,payload FROM trades ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
        return [
            {"ts": r[0], "symbol": r[1], "action": r[2], "payload": json.loads(r[3])}
            for r in rows
        ]
    ...
    def daily_realized_pnl(self, day=None) -> float:
        ...
        rows = self.db.execute(
            "SELECT payload FROM trades WHERE action='CLOSE' AND ts>=? AND ts<=?",
            (start, end),
        ).fetchall()
        total = 0.0
        for (payload,) in rows:
    ...
    def open_exposure(self) -> float:
        """Aggregate loss to each stored stop for currently open positions."""
