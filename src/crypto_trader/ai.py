...
class AIGateway:
    ...
    def __init__(self, base_url: str | None = None, api_key: str | None = None, model: str | None = None):
        ...
        self.base = (
            ...
            or os.getenv("KIOSAPI_BASE_URL")
            or ("https://router.kiosapi.com/v1" if kios_key else "")
        ).rstrip("/")
    ...
    def chat(self, system: str, user: str, tools: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        if not self.enabled():
            raise AIError("AI gateway is not configured")
        body = {
            "model": self.model,
            "messages": [
    ...
    def extract_plan(response: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            raise AIError("AI response has no message content") from exc
...
def decision_prompt(snapshot: dict[str, Any]) -> str:
    return (
        "Return JSON only. Required keys: decision, confidence, entry_low, entry_high, stop, "
        "take_profit_1, take_profit_2, thesis, invalidation, warnings. Use null for unavailable "
        "numeric fields.\nMARKET SNAPSHOT:\n"
