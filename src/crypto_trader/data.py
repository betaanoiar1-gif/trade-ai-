...
class BinancePublic:
    ...
    def klines(self, symbol="BTCUSDT", interval="1h", limit=500):
        ...
        if df[["open", "high", "low", "close", "volume"]].isna().any().any():
            raise MarketDataError("invalid numeric candle data")
        if not df.open_time.is_monotonic_increasing or df.open_time.duplicated().any():
            raise MarketDataError("non-monotonic or duplicate candles")
        if (df.high < df[["open", "close"]].max(axis=1)).any() or (df.low > df[["open", "close"]].min(axis=1)).any():
            raise MarketDataError("invalid OHLC geometry")
        if (df.volume < 0).any():
            raise MarketDataError("negative volume")
        return df

    def depth(self, symbol="BTCUSDT", limit=100):
        return _get(self.base + "/api/v3/depth", {"symbol": symbol, "limit": limit})

    def ticker_24h(self, symbol="BTCUSDT"):
        return _get(self.base + "/api/v3/ticker/24hr", {"symbol": symbol})
