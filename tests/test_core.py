...
from crypto_trader.ai import AIError, AIGateway
from crypto_trader.analysis import derivative_metrics, detect_regime, orderbook_metrics
...
def test_regime_and_orderbook():
    ...
    assert r.name in {"trend", "range", "high_volatility_range"}
    m = orderbook_metrics({"bids": [[100, 2], [99, 1]], "asks": [[101, 1], [102, 1]]})
    assert m["available"] and m["spread_bps"] > 0
...
def test_risk_long_and_short():
    ...
    for decision, stop in [("LONG", 95), ("SHORT", 105)]:
        p = TradePlan(
            "BTCUSDT", decision, 0.8, stop=stop,
            take_profit_1=110 if decision == "LONG" else 90,
        )
...
def test_paper_short_and_stop():
    b = PaperBroker()
    assert b.open("BTCUSDT", "short", 1, 100, 105)
    events = b.check_exits({"BTCUSDT": 106})
...
def test_ai_plan_extraction():
    r = {
        "choices": [{
            "message": {
                "content": "{\"decision\":\"WAIT\",\"confidence\":0.4,\"entry_low\":null,\"entry_high\":null,\"stop\":null,\"take_profit_1\":null,\"take_profit_2\":null,\"thesis\":\"wait\",\"invalidation\":\"none\",\"warnings\":[]}"
            }
        }]
    }
    p = AIGateway.extract_plan(r)
    assert p["decision"] == "WAIT"
    bad = {"choices": [{"message": {"content": "not json"}}]}
    try:
        AIGateway.extract_plan(bad)
        assert False
    except AIError:
        pass
